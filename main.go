...
func (k *keycloakAuth) ServeHTTP(rw http.ResponseWriter, req *http.Request) {
	...
	if err == nil && strings.HasPrefix(cookie.Value, "Bearer ") {
		...
		// match a regex on a specific claim
		if k.ConditionalAccessClaim != "" {
			conditionalAccessClaimValue, err := extractClaims(token, k.ConditionalAccessClaim)
			if err != nil {
				http.Error(rw, err.Error(), http.StatusInternalServerError)
				return
			} else {
				regex, err := regexp.Compile(k.ConditionalAccessRegexp)
				if err == nil {
					if !regex.MatchString(conditionalAccessClaimValue) {
						http.Error(rw, "Insufficient permissions", http.StatusForbidden)
						return
					}
				} else {
					http.Error(rw, "regex can not be compiled ", http.StatusInternalServerError)
					return
				}
			}
		}
