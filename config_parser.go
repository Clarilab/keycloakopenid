...
type keycloakAuth struct {
	...
	ConditionalAccessClaim  string
	ConditionalAccessRegexp string
}
...
func New(uctx context.Context, next http.Handler, config *Config, name string) (http.Handler, error) {
	...
	conditionalAccessClaim := ""
	if config.ConditionalAccessClaim != "" {
		conditionalAccessClaim = config.ConditionalAccessClaim
	}

	conditionalAccessRegexp := ".*"
	if config.ConditionalAccessRegexp != "" {
		conditionalAccessRegexp = config.ConditionalAccessRegexp
	}
	ignorePathPrefixes := []string{}
	...
	return &keycloakAuth{
		...
		ConditionalAccessClaim:  conditionalAccessClaim,
		ConditionalAccessRegexp: conditionalAccessRegexp,
